...
function betweenAvoidingRange(avoidingRanges, angle) {
  avoidingRanges.forEach((element) => {
    if (angle > Math.min(...element) && angle < Math.max(...element))
      return true;
  });
  return false;
}
...
export function AuthProvider({ children }) {
  ...
  const [pixelConverter, setPixelConverter] = useState(5.88);
  ...
  const getCanvasCoordinates = () => {
    const xranges = [
      canvasMargin + canvasPadding,
      width - (canvasMargin + canvasPadding + 2 * targetButtonRadius),
    ];
    const yranges = [
      canvasMargin + canvasPadding + scoreBoardHeight + scoreBoardBorderWidth,
      height - (canvasMargin + canvasPadding + 2 * targetButtonRadius),
    ];
  ...
  const RandomPoint = (prevPoint) => {
    ...
    if (prevPoint != undefined) {
      let limiter = 40;
      while (
        getDistance(prevPoint[0], prevPoint[1], x, y) <
          minDistance + targetButtonRadius &&
        limiter > 0
      ) {
  ...
  const RandomPointAtDistance = (prevPoint) => {
    ...
    if (prevPoint != undefined) {
      ...
      console.log(
        Math.fround(getDistance(x, y, prevPoint[0], prevPoint[1])) ==
          distanceRadius
      );
      return [x, y];
      //#endregion
  ...
  const saveData = () => {
    ...
    localStorage["pixelConverter"] = pixelConverter;
    localStorage["sessionCounter"] = sessionCounter;
  };
  ...
  const copyFromLastSaved = () => {
    ...
    setPixelConverter(localStorage["pixelConverter"] || 5.88);
    setSessionCounter(localStorage["sessionCounter"] + 1 || 1);
  };
  //#endregion
