...
function MainTaskActivity() {
  ...
  const handlePointClick2 = (event) => {
    ...
    try {
      ...
      document.getElementById("activity-wrapper").style.backgroundColor =
        "#4FC53C";
      setTimeout(() => {
        try {
          document.getElementById("activity-wrapper").style.backgroundColor =
  ...
  if (gameOver) {
    return (
      <EndGameTemplate
        ...
        avgWindow={true}
        averageTime={averageTime / (maxScore * 1000)}
        enableSigmaA={false}
      />
    );
  }
  ...
  return (
    ...
    >
      <PointGenerator
        posX={pos1X}
        posY={pos1Y}
        clickFunction={handlePointClick1}
        id={"point1"}
      />
      <PointGenerator
        posX={pos2X}
        posY={pos2Y}
        clickFunction={handlePointClick2}
        color={"white"}
        id={"point2"}
      />
    </ActivityTemplate>
